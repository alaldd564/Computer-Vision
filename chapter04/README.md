...
img_rgb = cv2.cvtColor(img_path, cv2.COLOR_BGR2RGB)
    ...
    > - 한 이미지를 다른 이미지에 정렬(align)하여 파노라마처럼 합성합니다.
    > - 변환 결과와 특징점 매칭 결과를 시각화합니다.

    **핵심 개념 및 자세한 설명**
    - **호모그래피(Homography)**: 두 평면(이미지) 사이의 변환 행렬 계산 (cv2.findHomography, RANSAC)
    - **cv2.warpPerspective**: 호모그래피 행렬로 이미지 변환
    - **파노라마 캔버스 동적 계산**: 두 이미지의 코너를 모두 변환해 최적의 캔버스 크기 산출
    - **빈 영역 제거**: 변환 후 남는 검은 영역을 자동으로 잘라내어 결과 이미지를 타이트하게 만듦
    - **특징점 매칭 시각화**: inlier만 선별하여 drawMatches로 표시
    ...
    **핵심 코드**
    ...
    - 왼쪽: inlier 매칭점만 표시한 SIFT 특징점 매칭 결과
    - 오른쪽: 호모그래피 변환 및 파노라마 합성 결과 (빈 영역 자동 제거)

    <div style="display: flex; flex-direction: row; gap: 20px;">
        <div style="text-align: center;">
            <b>03. SIFT Homography Alignment</b><br>
            <img src="03_sift_homography_alignment.png" alt="SIFT Homography Alignment 결과" width="400"/>
        </div>
    </div>
